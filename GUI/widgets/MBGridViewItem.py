...
# ── QPixmap LRU cache ────────────────────────────────────────────────────────
# Avoids re-converting PIL→QImage→QPixmap on every grid rebuild / re-sort.
...
class MusicBrowserGridItem(QFrame):
    ...
    def _render_image(self, pil_image: Image.Image) -> None:
        ...
        try:
            dpr = int(self.img_label.devicePixelRatioF() * 1000)
        except Exception:
            dpr = 1000
        cache_key = (self.artwork_id, Metrics.GRID_ART_SIZE, dpr, self._rounded_artwork)
        pixmap = _pixmap_cache_get(cache_key)
        if pixmap is None:
